...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  // allocate space for the current blocktile in SMEM (dynamic allocation)
...
void runKernel1_512(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C) {
  const uint K10_NUM_THREADS = 128;
  const uint K10_BN = 512;
  const uint K10_BM = 512;
  const uint K10_BK = 32;  // Fits in 99 KB opt-in shared memory (64 KB used)
  ...
  constexpr size_t sharedMemSize = (K10_BM * K10_BK + K10_BK * K10_BN) * sizeof(bf16);
  
  // Get kernel function pointer
  auto kernel = sgemmWarptiling<K10_BM, K10_BN, K10_BK, K10_WM, K10_WN, K10_WNITER, K10_TM, K10_TN, K10_NUM_THREADS>;
  
  // Enable opt-in shared memory (99 KB on Blackwell)
  ...
  kernel<<<gridDim, blockDim, sharedMemSize>>>(M, N, K, A, B, C);
}